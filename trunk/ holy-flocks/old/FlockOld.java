...
public class FlockOld {
	...
	public FlockOld(ArrayList<Mover> movers) {
		myMembers = movers;
		mySize = movers.size();
	}
	...
	// {
	// align(flockMember);
	// if (getAnnulus(flockMember, 1.5 * flockMember.getSize().width, 3 *
	// flockMember.getSize().width).size() > 0)
	// cohere(flockMember);
	// if (getNeighbors(flockMember, 1.5 * flockMember.getSize().width).size() >
	// 1)
	...
	// }
	// member.setVelocity(new PointD((xVelocity +
	// member.getVelocity().getX())/2,
	...
	// PointD velocity = member.getVelocity();
	// member.setVelocity(new PointD(velocity.getX() + 0.01 * (avgX -
	...
	// }
	//	
	// public ArrayList<Mover> getAnnulus(Mover member, double minR, double
	// maxR)
	// {
	//
