...
public class BouncerFactory extends Factory {
	 ...
	 * 
	 * @param canvas
	 *            within which shapes should appear
	 * @param numberToCreate
	 *            number of shapes to create
	 * @param trailSize
	 *            number of lines to draw following the shape
	 */
	public void createMovers(Canvas canvas, int number, int trailSize) {
		...
		FlockOld myFlock = new FlockOld();

		// create each BouncingBall and add to collection
		for (int k = 0; k < number; k++) {
			int size = canvas.nextIntInRange(MIN_SIZE, MAX_SIZE);

			BouncingSmiley temp = new BouncingSmiley(new PointD(canvas
					.nextIntInRange(size / 2, bounds.width - size / 2), canvas
					.nextIntInRange(size / 2, bounds.width - size / 2)),
					new Dimension(10, 10), new PointD(canvas
							.nextNonZeroIntInRange(-maxVelocity, maxVelocity),
							canvas.nextNonZeroIntInRange(-maxVelocity,
									maxVelocity)), new Color(canvas
							.nextIntInRange(0, 255), canvas.nextIntInRange(0,
							255), canvas.nextIntInRange(0, 255)), trailSize);
			myFlock.add(temp);
			canvas.add(myFlock);
			canvas.add(temp);
		}
